...
import { takePhotoAsync } from '@/hooks/use-image-picker';
...
export default function CaptureScreen() {
  ...
  const handleUpload = async () => {
    ...
    setLoading(false);
    // Navigate to results and pass uri and result as query params (result is JSON-stringified)
    router.push({ pathname: '/results', params: { uri, result: JSON.stringify(result), success: '1' } });
  };

  return (
    <View style={{ flex: 1 }}>
      <PaperAppbar title="Capture" />
      <View style={styles.container}>
        <Card style={{ width: '100%' }}>
          ...
          <Card.Actions>
            <Button icon="camera" mode="contained" onPress={handleTake}>
              Take Photo
            </Button>
            <View style={{ width: 12 }} />
